...
public class UI {
	...
	public static void printMatch(PartidaXadrez chessMatch, List<PecaXadrez> captured) {
		printBoard(chessMatch.pecasDoJogo());
		System.out.println();
		printCapturedPieces(captured);
		System.out.println();
		System.out.println("Turno : " + chessMatch.getTurno());
		if (!chessMatch.isCheckMate()) {
			System.out.println("Aguardando jogador: " + chessMatch.getJogadorAtual());
	...
	public static void printBoard(PecaXadrez[][] pieces, boolean[][] possibleMoves) {
		for (int i = 0; i < pieces.length; i++) {
			System.out.print((8 - i) + " ");
			for (int j = 0; j < pieces.length; j++) {
	...
	private static void printCapturedPieces(List<PecaXadrez> captured) {
		List<PecaXadrez> white = captured.stream().filter(x -> x.getCor() == Cor.BRANCO)
